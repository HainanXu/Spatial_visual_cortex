...
write_csv(df_full,"H07-0500_79205589_179_MBP")
...
library(plotly)
a<-ggplot(df_full,
   aes( com_x,  com_y,size=pixel_area))+ geom_point(shape = ".")+xlab("x position in pixel")+ylab("y position in pixel")+theme_bw()+geom_jitter(width = 0, height = 0.001, alpha = 0.5)+ theme(aspect.ratio = 1)

b<-ggMarginal(a,type="histogram",fill = "lightgrey",binwidth=10)
b
?ggMarginal
...
eK<-envelope(ln2, fun=Kest, nsim=200, nrank=1)
plot(eK)
```
