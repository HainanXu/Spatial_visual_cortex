...
Prepare the packages needed;download the correspongding packag.
```{R}
...
Due to limited biology knowledge, I assume we only interested in the large, black objects in the image.Let's convert the image to binary and perform a morphological operation.(to actually locate the position of doublet or triplet, I may consider only perform erode operation, and do not perform dialte)
```{R}
display(logim<0.99)
imOpened = opening(logim<0.99,
                             kern = makeBrush(7, shape = "disc"))
display(imOpened)
```

Next, I segmented the binary image into objects. Here is a table of the objects.0 represents the background, and the numbers from 1 to 169 represents for the objects that we identified.  
```{R}
